...
#define STEP_NOT_HERE_BUT_THERE(cur_turf, dirA, dirB) ((!CAN_STEP(cur_turf, get_step(cur_turf, dirA), simulated_only, caller, access, avoid) && CAN_STEP(cur_turf, get_step(cur_turf, dirB), simulated_only, caller, access, avoid)))
...
/datum/pathfind/jps/New(atom/movable/caller, access, max_distance, simulated_only, avoid, datum/callback/on_finish, atom/goal, mintargetdist, skip_first, diagonal_handling)
	. = ..()
...
/datum/pathfind/jps/finished()
	...
	switch(diagonal_handling)
		if(DIAGONAL_REMOVE_CLUNKY)
			path = remove_clunky_diagonals(path, caller, access, simulated_only, avoid)
		if(DIAGONAL_REMOVE_ALL)
			path = remove_diagonals(path, caller, access, simulated_only, avoid)
...
/datum/pathfind/jps/proc/lateral_scan_spec(turf/original_turf, heading, datum/jps_node/parent_node)
	...
	while(TRUE)
		...
		steps_taken++
		if(!CAN_STEP(lag_turf, current_turf, simulated_only, caller, access, avoid))
			return
