...
/atom/proc/set_light(l_range, l_power, l_color = NONSENSICAL_VALUE, l_angle, l_dir, l_on, l_base, l_height)
...
/// Setter for what, if any, base key this light is using
/atom/proc/set_light_base(new_value)
	if(new_value == light_base || light_flags & LIGHT_FROZEN)
		return
	if(SEND_SIGNAL(src, COMSIG_ATOM_SET_LIGHT_BASE, new_value) & COMPONENT_BLOCK_LIGHT_UPDATE)
		return
	. = light_base
	light_base = new_value
	SEND_SIGNAL(src, COMSIG_ATOM_UPDATE_LIGHT_BASE, .)
	return .
