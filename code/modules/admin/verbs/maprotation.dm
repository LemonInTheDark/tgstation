ADMIN_VERB(admin_change_map, R_SERVER, "Change Map", "Set the next map.", ADMIN_CATEGORY_SERVER)
	...
	if(chosenmap != "Custom")
		...
		if (SSmap_vote.set_next_map(virtual_map))
			...
			// Stinky smelly dodo I'm sorry
			if(!fexists(PATH_TO_NEXT_MAP_JSON))
				CRASH("set_next_map's behavior changed and next_map.json isn't where we thought it was (sorry this shit is dumb)")
			var/list/map_json = json_decode(file2text(PATH_TO_NEXT_MAP_JSON))
			fdel(PATH_TO_NEXT_MAP_JSON)
			map_json["admin_forced"] = TRUE
			text2file(json_encode(map_json), PATH_TO_NEXT_MAP_JSON)
		return
	...
	var/should_upload_config = tgui_alert(user,"Would you like to upload an additional config for this map?", "Map Config", list("Yes", "No"))
	if(should_upload_config == "Yes")
	...
	if(fexists(PATH_TO_NEXT_MAP_JSON))
		fdel(PATH_TO_NEXT_MAP_JSON)
	// I'm just gonna say I don't like this code, we're making assumptiosn about how set_next_map()
	// works and it makes us prone to weird errors
