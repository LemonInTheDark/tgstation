/*
	...
	The on-station prophunt area is of type [holodeck_type].
	...
	1) Define new areas for the prophunt programs
	2) Map them
...
/obj/machinery/computer/prophunt
	name = "prophunt control console"
	desc = "A computer used to control a nearby prophunt."
	...
	var/area/prophunt/linked
	var/area/prophunt/program
	var/area/prophunt/last_program
	var/area/offline_program = /area/prophunt/mg2/offline
	...
	var/holodeck_type = /area/prophunt/mg2	// locate(this) to get the target prophunt
	var/program_type = /area/prophunt/mg2	// subtypes of this (but not this itself) are loadable programs
...
/obj/machinery/computer/prophunt/Initialize(mapload)
	..()
	return INITIALIZE_HINT_LATELOAD

/obj/machinery/computer/prophunt/LateInitialize()
	...
	if(!linked || !offline_program)
		log_world("No matching prophunt area found")
		qdel(src)
		return
	var/area/AS = get_area(src)
	if(istype(AS, /area/prophunt))
		log_mapping("Holodeck computer cannot be in a prophunt, This would cause circular power dependency.")
		qdel(src)
		return
	else
		linked.linked = src
...
/obj/machinery/computer/prophunt/Destroy()
	emergency_shutdown()
	if(linked)
		linked.linked = null
		linked.power_usage = new /list(AREA_USAGE_LEN)
...
/obj/machinery/computer/prophunt/power_change()
	. = ..()
	toggle_power(!machine_stat)

/obj/machinery/computer/prophunt/ui_interact(mob/user, ui_key = "main", datum/tgui/ui = null, force_open = FALSE, datum/tgui/master_ui = null, datum/ui_state/state = GLOB.default_state)
	ui = SStgui.try_update_ui(user, src, ui_key, ui, force_open)
	if(!ui)
		ui = new(user, src, ui_key, "Holodeck", name, ui_x, ui_y, master_ui, state)
		ui.open()

/obj/machinery/computer/prophunt/ui_data(mob/user)
	var/list/data = list()
...
/obj/machinery/computer/prophunt/ui_act(action, params)
	if(..())
		return
	. = TRUE
	switch(action)
...
/obj/machinery/computer/prophunt/process()
	if(damaged && prob(10))
	...
	if(!floorcheck())
		...
		for(var/mob/M in urange(10,src))
			M.show_message("The prophunt overloads!")
...
/obj/machinery/computer/prophunt/emag_act(mob/user)
	if(obj_flags & EMAGGED)
		return
...
/obj/machinery/computer/prophunt/emp_act(severity)
	. = ..()
	if(. & EMP_PROTECT_SELF)
		return
	emergency_shutdown()

/obj/machinery/computer/prophunt/ex_act(severity, target)
	emergency_shutdown()
	return ..()

/obj/machinery/computer/prophunt/blob_act(obj/structure/blob/B)
	emergency_shutdown()
	return ..()

/obj/machinery/computer/prophunt/proc/generate_program_list()
	for(var/typekey in subtypesof(program_type))
		var/area/prophunt/A = GLOB.areas_by_type[typekey]
		if(!A || !A.contents.len)
			continue
...
/obj/machinery/computer/prophunt/proc/toggle_power(toggleOn = FALSE)
	if(active == toggleOn)
		return
...
/obj/machinery/computer/prophunt/proc/emergency_shutdown()
	last_program = program
	load_program(offline_program, TRUE)
	active = FALSE

/obj/machinery/computer/prophunt/proc/floorcheck()
	for(var/turf/T in linked)
		if(!T.intact || isspaceturf(T))
			return FALSE
	return TRUE

/obj/machinery/computer/prophunt/proc/nerf(active)
...
/obj/machinery/computer/prophunt/proc/load_program(area/A, force = FALSE, add_delay = TRUE)
	if(!is_operational())
		A = offline_program
		force = TRUE
...
/obj/machinery/computer/prophunt/proc/finish_spawn()
	var/list/added = list()
...
/obj/machinery/computer/prophunt/proc/derez(obj/O, silent = TRUE, forced = FALSE)
