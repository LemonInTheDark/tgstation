...
/datum/component/overlay_lighting/Initialize(_range, _power, _color, starts_on, is_directional, is_beam, force)
	...
	RegisterSignal(light, COMSIG_LIGHT_OVERLAY_UPDATE_HOLDER, PROC_REF(holder_changed))
...
/datum/component/overlay_lighting/proc/holder_changed(datum/source, atom/old_holder, atom/new_holder)
	SIGNAL_HANDLER
	if(old_holder)
		UnregisterSignal(old_holder, list(COMSIG_MOVABLE_MOVED, COMSIG_LIGHT_EATER_QUEUE))
